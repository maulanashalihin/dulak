...
export class Inertia {
  constructor(private c: InertiaContext, private assets: InertiaAssets) {}
  ...
  page(component: string, props: Record<string, unknown> = {}, errors?: Record<string, string>): Page {
    let pageProps = props
    if (this.c.headers['x-inertia-partial-component'] === component) {
      ...
      if (only) {
        pageProps = Object.fromEntries(Object.entries(props).filter(([k]) => only.includes(k)))
      }
      if (except) {
  ...
  error(component: string, errors: Record<string, string>, status = 422): Response {
  ...
  redirect(path: string, status: 302 | 303 = 303): Response {
    return Response.redirect(new URL(path, this.c.request.url).toString(), status)
  }
  ...
  private locationVisit(): Response {
    return new Response(null, {
      status: 409,
      headers: {
        'x-inertia-location': new URL(this.currentUrl, this.c.request.url).toString(),
        'x-inertia-version': this.assets.version,
      },
    })
  }
