...
import { createGoogleUser, findUserByEmail, findUserByGoogleId, linkGoogleAccount, type UserRow } from '../db'
...
async function fetchProfile(accessToken: string): Promise<GoogleProfile> {
  ...
  if (!data.id || !data.email) throw new Error('Google profile is missing id/email')
  return {
    id: data.id,
...
async function findOrCreateGoogleUser(profile: GoogleProfile): Promise<UserRow> {
  ...
  const created = createGoogleUser.get(profile.name, profile.email, profile.id, profile.picture ?? '')
...
export const oauthRoutes = (assets: InertiaAssets) =>
  new Elysia()
    ...
    .get('/auth/google/callback', async ({ request, cookie, store }) => {
      const url = new URL(request.url)
      const code = url.searchParams.get('code')
      const state = url.searchParams.get('state')
      const stored = typeof cookie.oauth_state?.value === 'string' ? cookie.oauth_state.value : null
      clearOAuthStateCookie(cookie.oauth_state)

      if (url.searchParams.get('error') || !code || !state || !stored || state !== stored) {
        return Response.redirect(new URL('/login?notice=google_failed', request.url).toString())
      }
      try {
