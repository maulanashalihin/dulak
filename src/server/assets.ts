 ...
 *  2. Bun.build bundles the Vue client (app.ts) with the Vue plugin
 *  3. Bun.build bundles the SSR entry (ssr.ts) → dist/ssr.js
...
export async function buildClientAssets(): Promise<void> {
	...
	// 2. Client bundle: Vue SFCs compiled for browser.
	...
	// 3. SSR bundle: Vue compiled for server-side rendering. Loaded lazily
