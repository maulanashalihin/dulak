 ...
 *  3. Bun.build bundles the SSR entry (ssr.ts) with generate: 'server'
 *  4. dist/assets/* (content-hashed) + dist/manifest.json + dist/ssr.js
...
export async function buildClientAssets(): Promise<void> {
	...
	// 3. SSR bundle: Svelte compiled for server-side rendering.
	await Bun.build({
