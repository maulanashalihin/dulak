/**
 * End-to-end test suite: boots the full app (Elysia + bun:sqlite + Inertia)
 * against an in-memory database and drives it via app.handle().
 * Run with: bun test
 */
...
async function registerUser(email: string, password = 'password123'): Promise<string> {
...
describe('auth basics', () => {
  ...
  it('rejects duplicate email', async () => {
    ...
    expect(res.status).toBe(422)
    expect((await page(res)).props.errors.email).toBe('That email is already registered.')
  })
  ...
  it('logs out: session is destroyed server-side', async () => {
    ...
    const res = await call('/logout', { method: 'POST', headers: { ...xhr, cookie } })
    expect(res.status).toBe(303)
    expect(new URL(res.headers.get('location')!).pathname).toBe('/login')
...
describe('inertia protocol', () => {
  it('returns 409 + X-Inertia-Location on version mismatch', async () => {
    ...
    expect(res.headers.get('x-inertia-location')).toBe('http://localhost:3000/dashboard')
  })
...
describe('roles & admin', () => {
  ...
  it('serves paginated users to admins', async () => {
    ...
    const res = await call('/admin', { headers: { cookie: adminCookie, ...xhr } })
    expect(res.status).toBe(200)
    const data = await page(res)
    ...
    expect(data.props.users.data.some((u: any) => u.email === 'boss@example.com')).toBe(true)
...
describe('password reset', () => {
  ...
  it('resets a password end to end (log mail driver)', async () => {
    ...
    const badConfirm = await call('/reset-password', {
      method: 'POST',
      headers: xhr,
      body: { email: 'resetflow@example.com', token, password: 'newpassword123', passwordConfirmation: 'other' },
    })
    expect(badConfirm.status).toBe(422)
    expect((await page(badConfirm)).props.errors.password).toContain('does not match')

    const reset = await call('/reset-password', {
      method: 'POST',
      headers: xhr,
      body: { email: 'resetflow@example.com', token, password: 'newpassword123', passwordConfirmation: 'newpassword123' },
    })
    expect(reset.status).toBe(303)
    expect(new URL(reset.headers.get('location')!).searchParams.get('notice')).toBe('password_reset')
  ...
  it('rejects expired/invalid reset tokens', async () => {
    ...
    expect(res.status).toBe(422)
    expect((await page(res)).props.errors.token).toContain('invalid or has expired')
  })
})
