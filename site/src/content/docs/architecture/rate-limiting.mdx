...
```mermaid
flowchart LR
  Request --> Global["Global limiter<br/>200 req / 60s / IP"]
  Global -->|pass| Routes
  Global -->|429| Blocked["Too many requests"]
  Routes --> AuthRoutes["Auth routes<br/>30 req / 60s / IP"]
  AuthRoutes -->|429| Blocked
  AuthRoutes -->|pass| Handler
```
