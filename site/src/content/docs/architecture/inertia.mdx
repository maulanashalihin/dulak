...
Adapter ──► X-Inertia-Version matches?
              │
              ├─ yes ──► JSON page payload ──► Browser
              └─ no  ──► 409 + X-Inertia-Location ──► full reload ──► Browser

Adapter ──► browser visit ──► SSR enabled and guest?
              │
              ├─ yes ──► renderToString → full HTML + data-server-rendered ──► Browser
              └─ no  ──► empty shell + JSON (client renders from scratch) ──► Browser
```
