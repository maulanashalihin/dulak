...
Dulak is a production-grade full-stack boilerplate running entirely on **Bun**:

- **Hono 4.x** — HTTP layer, served via `Bun.serve`
- **bun:sqlite** — synchronous, zero-ORM database
- **Inertia v3** — server-driven UI with in-process SSR (React 19, Svelte 5, or Vue 3)
...
- Rate limiting, CSRF, security headers, batched request logging, gzip, Prometheus metrics
- 67-test E2E suite (`bun test --isolate`)
