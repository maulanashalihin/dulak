...
tags:
  ...
  - performance
  - philosophy
excerpt: "A SPA ships an empty div and a JS bundle. SSR ships HTML on the first byte. The difference is not theoretical — it's 50ms vs 2 seconds on a budget phone, and it's whether Google can read your page."
---

The [Inertia post](/blog/why-inertia-not-spa/) covered the architecture: one router, one validator, no API layer, no client-side state. This post is about something different — the rendering strategy. Why does Dulak render HTML on the server instead of shipping a JavaScript bundle to the client?

These are separate decisions. You can use Inertia without SSR (ship an empty shell, let the client render). You can use SSR without Inertia (Next.js, Nuxt). Dulak does both, but the rendering argument stands on its own.
