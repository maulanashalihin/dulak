...
The default is zero-dependency, zero-build-step, readable by anyone who knows CSS. The upgrade path to Tailwind is two dependencies and an hour of migration. The downgrade path from Tailwind is a rewrite. Pick the default that's cheapest to change — and if you prefer Tailwind, `bun create dulak@latest my-app --template react-tailwind` is right there. See the [installation guide](/getting-started/installation/) for all available templates.
