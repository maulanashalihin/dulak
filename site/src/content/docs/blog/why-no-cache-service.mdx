---
title: "Why Dulak doesn't ship a cache service"
date: 2026-08-08
authors:
  - maulana
tags:
  ...
  - sqlite
  - philosophy
excerpt: "SQLite with WAL mode serves 52K reads/s on a single process. Adding Redis would be a dependency that solves a problem Dulak doesn't have."
---
...
bun:sqlite with WAL mode serves **52,000 reads per second** and **95,000 writes per second** on a single Bun process. That's not a synthetic micro-benchmark — it's the full Dulak app with auth, sessions, and request routing overhead included.
