...
and 29.8K reads/s** — 7× and 19× slower. Even with `synchronous_commit
= off` (Postgres' version of the `NORMAL` tradeoff), writes stay 3×
behind. See the [full numbers](/database/performance/#sqlite-vs-postgres-on-the-same-machine).
...
The numbers: [52K reads/s via HTTP, 19.6K creates/s via HTTP, 95K writes/s direct](/database/performance/) on a laptop. The backup: copy a file. The swap: architectural, not performance — and scheduled, not emergency. The cost: zero infrastructure until you need it.
