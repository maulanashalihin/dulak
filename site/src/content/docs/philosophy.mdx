...
that require the next maintainer to reverse-engineer intent. Whoever
comes next — human or AI agent — should be able to understand the code,
change it, and not be afraid of breaking it.

## Deliberately boring

Every choice trades "clever" for "obviously right". When two approaches
exist, only the simpler one is kept. For example, route handlers are
written inline in their route file instead of being
...
The infrastructure a deployed app needs — CSRF, two-layer rate limiting
(global DDoS baseline + auth brute-force), security headers, versioned
migrations, graceful shutdown, Docker, CI — is wired and tested from day
one, not scaffolded. What is missing is your business logic, and that is
the point: you start from a skeleton that already works, not one you
have to harden.
...
Bun 1.4+, Inertia v3) and upgrades only when there is a concrete reason. A
