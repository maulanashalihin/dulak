...
Convention for CRUD form submissions in dulak-v2 using Inertia.js v3 + Vue 3
...
> dulak-v2 `Profile.vue` uses `info.value.patch('/profile')` (not `put`) and
...
| `@change="form.clearErrors('field')"` | Clears a field error as soon as the user edits it (dulak-v2 convention) |
...
- dulak-v2: `src/client/pages/Login.vue`, `Register.vue`, `Profile.vue`
