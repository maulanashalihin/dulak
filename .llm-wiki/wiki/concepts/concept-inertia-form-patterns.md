...
Convention for CRUD form submissions in dulak-v2 using Inertia.js v3 + React 19
...
Least boilerplate. Not currently used in dulak-v2 but valid for throwaway
admin forms.
...
> dulak-v2 `Profile.tsx` uses `info.patch("/profile")` (not `put`) and keeps
...
| `clearErrors("field")` in `onChange` | Clears a field error as soon as the user edits it (dulak-v2 convention) |
...
- dulak-v2: `src/client/pages/Login.tsx`, `Register.tsx`, `Profile.tsx`
